...
model {
  ...
  target += std_normal_lpdf(sigma_time);
  target += std_normal_lpdf(sigma_eps);
  ...
  // Time Effects (Linear Trend with Error)
  target += std_normal_lpdf(drift);
  target += std_normal_lpdf(time_raw);
  
  //Prior for rho
  target += beta_lpdf(rho|0.5,0.5); // prior as proposed by Riebler et. al 
  
  //Age Effect
  //target += normal_lpdf(beta_age|0,sigma_age); // IID NORMAL PRIOR 
